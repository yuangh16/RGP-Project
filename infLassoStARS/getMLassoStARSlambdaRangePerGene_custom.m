function [minLambdas, maxLambdas, maxedOut, notSmallEnough,...
  ...
  getMLassoStARSlambdaRangePerGene_custom(predictorMat,...
...
for res = 1:totResponses % can be a parfor loop
    ...
    for ss = 1:totSS
        subsamp = subsamps(ss,:);
%         currPreds = zscore(squeeze(predictorMat(res,predInds,subsamp))');% nobs X nvars
%         currResponses = zscore(responseMat(res,subsamp)'); % nobs X 1
        currPreds = (squeeze(predictorMat(res,predInds,subsamp))'); % nobs X nvars
        currResponses = (responseMat(res,subsamp)'); % nobs X 1
