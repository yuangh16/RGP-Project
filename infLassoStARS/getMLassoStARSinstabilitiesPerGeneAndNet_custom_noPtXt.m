function [geneInstabilities,netInstabilities,ssMatrix] =...
    getMLassoStARSinstabilitiesPerGeneAndNet_custom_noPtXt(predictorMat,...
    responseMat,priorWeightsMat,lambdaRange,subsampleSize, totSS)
%modifications: 
%1. remove the zscore inside functions
...
for res = 1:totResponses 
    ...
    for ss = 1:totSS
        subsamp = subsamps(ss,:);
        %currPreds = zscore(predictorMat(predInds,subsamp)'); % removed zscore
        %currResponses = zscore(responseMat(res,subsamp)'); % removed zscore
        currPreds = (predictorMat(predInds,subsamp)'); % removed zscore
        currResponses = (responseMat(res,subsamp)'); % removed zscore
