function calcR2predFromStabilities_custom_PtXt(instabMat,stabilitiesMat,r2OutMat,modSizes)
...
load([instabMat '.mat']) % load instabiliaty mat
load([stabilitiesMat '.mat']) %load network stability mat

[xx, instabTitleInf, ext] = fileparts(instabMat); %fileparts(FILE) returns the path, file name, and file name extension for the specified FILE
instabTitleInf = strrep(strrep(strrep(instabTitleInf,'instRange_63',''),'ZOOM',''),'_',' '); % strrep find and replace substring, strrep(ORIGTEXT,OLDSUBTEXT,NEWSUBTEXT) 

totPredTargs = length(targGenes); %targets genes number
totConds = length(conditionsc); % samples number
totPreds = length(allPredictors); % TF number
% modSizes = [.25 .5 .75 1:25]; % desired model sizes
totModSizes = length(modSizes); %modSizes = 1:meanEdgesPerGene
instabRangeNet = zeros(totModSizes,1);

[stabOrd] = sort(abs(allStabsMergedTFs(find(allStabsMergedTFs))),'descend'); % make sure cutoffs are in order, the stability from previous step
totInts = length(stabOrd); % find(X) returns indices of the array X that point to nonzero elements

subsampleCutoffs = modSizes; % get actual model cutoffs
rmInds = [];
for ms = 1:totModSizes
    currNetSize = round(totPredTargs*modSizes(ms)); %the network size = TFpergene*genes number
    if currNetSize <= totInts
        subsampleCutoffs(ms) = stabOrd(currNetSize); % subsample the network according stability rank,to achieve , such as, 1TF/genes
        currTheta = round(subsampleCutoffs(ms))/totSS;
        instabRangeNet(ms) = 2*currTheta*(1-currTheta); % get instability at the cutoff
    else %if the TF/gene model size interaction > the real total interactions (nonzero) at network, remove these model size
        rmInds = [rmInds; ms];ls
        lsllll
...
SSE_test = zeros(totModSizes,totPredTargs);

kcount = 0;

predictorMat = predictorMat_rna;
...
%     disp(instabTitleInf)
    ...
    totTrainConds = length(trainInds); % number of training samples
    testInds = setdiff(1:totConds,trainInds); % index of test samples
    totTestConds = length(testInds); %number of test samples
    targGenesTrain = responseMat(:,trainInds); % target genes expression for training samples     
    targGenesTest = responseMat(:,testInds); % target genes expression of test samples
    tfaTrain = predictorMat(:,trainInds); % TFA of training samples
    tfaTest = predictorMat(:,testInds); % TFA of test samples
    % mean-center training and test target genes, according to training mean
    meanTargGene = mean(targGenesTrain,2); %mean target genes expression of training samples       
    %cTargGenesTrain = targGenesTrain - repmat(meanTargGene,1,totTrainConds); %mean center train
    %cTargGenesTest = targGenesTest - repmat(meanTargGene,1,totTestConds); % mean center test 
        
    %cTargGenesTrain = 1 ./(1+exp(-(targGenesTrain - repmat(meanTargGene,1,totTrainConds)))); %logistic for centered
    %cTargGenesTest = 1 ./(1+exp(-(targGenesTest - repmat(meanTargGene,1,totTestConds)))); % logistic for centered
    
    cTargGenesTrain = targGenesTrain ; %use input expression
    cTargGenesTest = targGenesTest ; % use input expression
    % z-score TFA, according to training mean and std
    meanTfa = mean(tfaTrain,2); %mean TFA from training samples
    stdTfa = std(tfaTrain')'; %std TFA from training samples
    %zTfaTrain = (tfaTrain - repmat(meanTfa,1,totTrainConds))./repmat(stdTfa,1,totTrainConds); % mean center + scale train
    %zTfaTest = (tfaTest - repmat(meanTfa,1,totTestConds))./repmat(stdTfa,1,totTestConds); % mean center + scale test        
    
    zTfaTrain = (tfaTrain); % use input tfa
    zTfaTest = (tfaTest); % use input tfa      
    ...
    %baseModelTest = 1 ./(1+exp(-(baseModelTest)));%logistic %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    baseModelTrain = cTargGenesTrain;
    %baseModelTrain = 1 ./(1+exp(-(baseModelTrain)));%logistic %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % SSE of train data mean for test data (we'll calculate R^2
    % = 1 - [ sum(SSE_predMat) / sum(SSEpredMatMean) ]
    SSE_predMatMean(testInds,:) = (baseModelTest-mean(baseModelTrain,2))'.^2; %(xij-xi,train)^2
    ...
    SSE_fitMatMean(trainInds,:) = SSE_fitMatMean(trainInds,:) + ...
        (baseModelTrain-mean(baseModelTrain,2))'.^2;        
        
    currSS = zeros(totPreds,1);
    for targ = 1:totPredTargs % use for to calculate the SSE for every genes
        % start with least stringent to most stringent instability cutoff
        currTargName = targGenes{targ}; % name for this target gene
        currTargValsTrain = cTargGenesTrain(targ,:)'; % centered expression of this target gene at train samples
        currTargValsTest = cTargGenesTest(targ,:)'; % centered expression of this target gene at test samples
        %currTargValsTest = 1 ./(1+exp(-(currTargValsTest))); %  logistic%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %transform
        currTargValsTrain_1ogistic = zeros(size(currTargValsTrain));
        %currTargValsTrain_1ogistic = 1 ./(1+exp(-(currTargValsTrain)));% logistic %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

        currSubsamples = allStabsMergedTFs(targ,:); % the stability of this target gene with each TF.
        
        for iind = 1:totModSizes
            currCut = subsampleCutoffs(iind);
            
            intInds = find(currSubsamples>=currCut); % get the TF/gene interaction > cutoff from TF per gene, for this target gene
            % get target expression levels
            totParams = length(intInds); % number of total edges pass threshold
            ...
            if totParams > 0 % make sure there's at least one feature for regression
                currPredValsTrain = zTfaTrain(intInds,:)'; % test samples' tfa
                currPredValsTest = zTfaTest(intInds,:)'; % train samples' tfa
                % fit a linear model with least squares (use pseudo
                % inverse, just in case we end up in the under-determined
                % regime)
                coefs = currPredValsTrain \ (currTargValsTrain-mean(currTargValsTrain)); % this is how B calculated ?
                trainPreds = currPredValsTrain * coefs+mean(currTargValsTrain);
                testPreds = currPredValsTest * coefs+mean(currTargValsTrain);
                
                %coefs = currPredValsTrain \ (currTargValsTrain); % this is how B calculated ?
                %trainPreds = currPredValsTrain * coefs;
                %testPreds = currPredValsTest * coefs;
                %testPreds = 1 ./(1+exp(-(testPreds)));% logistic %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                %trainPreds = 1 ./(1+exp(-(trainPreds)));% logistic %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                 
                %testPreds = -log(1./testPreds-1); %reverse logistic
                %transfrom               
                ...
                SSE_fitMatNz(iind) = SSE_fitMatNz(iind) + ... %fit is the train samples as test samples
            ...
            %    (trainPreds'-currTargValsTrain_1ogistic').^2; %%%%%%%%%%%%%%%%%%%%logistic transform
            SSE_fitMat(iind,trainInds,targ) = SSE_fitMat(iind,trainInds,targ) + ...
...
%     mkdir(figOutBase)
    ...
    for bind = 1:totModSizes
        ...
        %add custom r2 for divide before sum
%         SSE_divide = (currSSE_preds./SSE_predMatMean);
%         SSE_divide_1 = SSE_divide (testInds,:);
%         size_divide = size(SSE_divide_1);
%         r2_pred_custom(bind) = (sum(SSE_divide_1(:)))/(size_divide(1)*size_divide(2));  
%         r2_pred_byTarg_custom(bind,:) = (sum(SSE_divide_1)./size_divide(1))';% got NaN for the zeros in SSE_predMat
%            
...
%         link = linkage(pdis,'average');
    ...
    hold on 
    plot(totEdges,r2_fit,'g-','LineWidth',lineWidth)
    legend({'Overall R^2_{pred}','Gene Median R^2_{pred}','Gene Average R^2_{pred}','R_2 fit'},'Location','EastOutside')
    axis([0 30 -0.2 1])
...
%     figure
%     totEdges = sum(numParams,2)/totPredTargs;
%     plot(totEdges,r2_pred_custom,'k-','LineWidth',2)
%     hold on, grid on, set(gca,'FontSize',fontSize,'FontWeight','Bold')
%     plot(median(numParams,2),median(r2_pred_byTarg_custom,2),'b-','LineWidth',lineWidth)
%     hold on
%     plot(mean(numParams,2),mean(r2_pred_byTarg_custom,2),'r-','LineWidth',lineWidth)
