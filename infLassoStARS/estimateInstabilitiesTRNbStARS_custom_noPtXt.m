function estimateInstabilitiesTRNbStARS_custom_noPtXt(geneExprMat,tfaMat,lambdaBias,tfaOpt,...
...
if strcmp(tfaOpt,'_TFmRNA') % use the mRNA levels of TFs
...
if strcmp(tfaOpt,'_accTFA') % use the mRNA levels of TFs
    predictorMat = [nc_acctfa; potRegMat_mRNA(uniNoPriorRegInds,:)];
    disp(['accTFA used.'])
end

priorWeightsMat = ones(totTargGenes,totPreds) - (1-lambdaBias)*abs(sign(priorMat));

if strcmp(tfaOpt,'_TFmRNA')
...
% get lambda range 19471 zhaoz18   20   0   13.1g   4.1g   5704 S  51.5  1.6 932:24.71 MATLAB                                                                                             
[minLambdas, maxLambdas, maxedOut, notSmallEnough,...
    ...
    getMLassoStARSlambdaRangePerGene_custom_noPtXt(predictorMat(:,trainInds),responseMat(:,trainInds),...
...
if log10(maxLambdaNet)-log10(minLambdaNet)< lamLog10step
logLamRange =  [log10(minLambdaNet),log10(minLambdaNet)+lamLog10step];
end
lambdaRange = [10.^logLamRange]';
...
[geneInstabilities,netInstabilities,ssMatrix] = ...
    getMLassoStARSinstabilitiesPerGeneAndNet_custom_noPtXt(predictorMat(:,trainInds),...
