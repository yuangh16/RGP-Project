function estimateInstabilitiesTRNbStARS_custom(geneExprMat,tfaMat,PtXt_matrix,lambdaBias,tfaOpt,...
...
predictorMat_rna = [medTfas; potRegMat_mRNA(uniNoPriorRegInds,:)];
if tfaOpt % use the mRNA levels of TFs
    ...
    predictorMat_rna = currPredMat;
    disp(['TF mRNA used.'])
end


predictorMat = PtXt_matrix;
...
[minLambdas, maxLambdas, maxedOut, notSmallEnough,...
    ...
    getMLassoStARSlambdaRangePerGene_custom(predictorMat(:,:,trainInds),responseMat(:,trainInds),...
...
[geneInstabilities,netInstabilities,ssMatrix] = ...
    getMLassoStARSinstabilitiesPerGeneAndNet_custom(predictorMat(:,:,trainInds),...
...
save([instabOutMat '.mat'],'-v7.3',...
    'geneInstabilities','netInstabilities','ssMatrix','predictorMat','predictorMat_rna',...
