function integratePrior_estTFA(geneExprMat,priorFile,acctfaFile,edgeSS,...
     minTargets, tfaOutMat)
%% integratePrior_estTFA(geneExprMat,priorFile,edgeSS,...
%      minTargets, tfaOutMat)
%% GOALS: 
% add one more case for acctfa
...
mergedTFsExist = 1;
try
    mergedFile = strrep(priorFile,'.txt','_merged.txt');
    mergedTFs = strrep(priorFile,'.txt','_mergedTFs.txt');
    ls(mergedFile)
    ls(mergedTFs)
catch
    mergedTFsExist = 0;
...
fclose(fid);
potRegs_acctfa = C{1};
ncounts_acctfa = [C{2:end}];
[vars obs] = size(ncounts_acctfa);

% Limit prior regulators and gene targets to those included in input data (.mat)
[pRegsacctfa, pRegIndsTmp, potRegInds] = intersect(pRegsTmp,potRegs);
nc_acctfa = ncounts_acctfa(pRegIndsTmp,:); 
