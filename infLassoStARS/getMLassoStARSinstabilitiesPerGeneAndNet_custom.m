function [geneInstabilities,netInstabilities,ssMatrix] =...
    getMLassoStARSinstabilitiesPerGeneAndNet_custom(predictorMat,...
...
for res = 1:totResponses 
    ...
    for ss = 1:totSS
        subsamp = subsamps(ss,:);
%         currPreds = zscore(squeeze(predictorMat(res,predInds,subsamp))');
%         currResponses = zscore(responseMat(res,subsamp)');
        currPreds = (squeeze(predictorMat(res,predInds,subsamp))');
        currResponses = (responseMat(res,subsamp)');
